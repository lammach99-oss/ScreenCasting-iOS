...
final class VideoContentViewportTests: XCTestCase {
    func testNativeIpadM2FrameFillsMatchingFullWindowGeometry() throws {
        let viewport = try XCTUnwrap(
            VideoContentViewport.aspectFit(
                containerSize: CGSize(width: 1_194, height: 834),
                videoSize: CGSize(width: 2_388, height: 1_668)))

        XCTAssertEqual(viewport.rect.origin.x, 0, accuracy: 0.000_001)
        XCTAssertEqual(viewport.rect.origin.y, 0, accuracy: 0.000_001)
        XCTAssertEqual(viewport.rect.width, 1, accuracy: 0.000_001)
        XCTAssertEqual(viewport.rect.height, 1, accuracy: 0.000_001)
    }

    func testLetterboxTouchOutsideRenderedContentIsIgnored() throws {
        let viewport = try XCTUnwrap(
            VideoContentViewport.aspectFit(
                containerSize: CGSize(width: 1_600, height: 1_200),
                videoSize: CGSize(width: 1_600, height: 900)))

        XCTAssertNil(
            viewport.normalizedPoint(
                for: CGPoint(x: 800, y: 149),
                in: CGRect(x: 0, y: 0, width: 1_600, height: 1_200)))
    }

    func testRenderedContentCenterMapsToNormalizedCenter() throws {
        let viewport = try XCTUnwrap(
            VideoContentViewport.aspectFit(
                containerSize: CGSize(width: 1_600, height: 1_200),
                videoSize: CGSize(width: 1_600, height: 900)))
        ...
        let point = try XCTUnwrap(
            viewport.normalizedPoint(
                for: CGPoint(x: 800, y: 600),
                in: CGRect(x: 0, y: 0, width: 1_600, height: 1_200)))

        XCTAssertEqual(point.x, 0.5, accuracy: 0.000_001)
        XCTAssertEqual(point.y, 0.5, accuracy: 0.000_001)
    }

    func testRenderedContentCornersMapToNormalizedCorners() throws {
        let viewport = try XCTUnwrap(
            VideoContentViewport.aspectFit(
                containerSize: CGSize(width: 1_600, height: 1_200),
                videoSize: CGSize(width: 1_600, height: 900)))
        let bounds = CGRect(x: 0, y: 0, width: 1_600, height: 1_200)

        let topLeft = try XCTUnwrap(
            viewport.normalizedPoint(for: CGPoint(x: 0, y: 150), in: bounds))
        let bottomRight = try XCTUnwrap(
            viewport.normalizedPoint(for: CGPoint(x: 1_600, y: 1_050), in: bounds))

        XCTAssertEqual(topLeft.x, 0, accuracy: 0.000_001)
        XCTAssertEqual(topLeft.y, 0, accuracy: 0.000_001)
        XCTAssertEqual(bottomRight.x, 1, accuracy: 0.000_001)
        XCTAssertEqual(bottomRight.y, 1, accuracy: 0.000_001)
    }
}
